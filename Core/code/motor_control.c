...
#define PPDD_KP2         1.50f      // 二次位置比例系数
#define PPDD_KD1         0.5f       // 位置微分系数
#define PPDD_GKD         0.5f       // 角速度微分系数
...
// 速度环PI参数（目标速度 -> PWM输出）
#define L_SPEED_KP            1.5f       // 左电机速度比例系数
#define L_SPEED_KI            0.4f       // 左电机速度积分系数
#define L_SPEED_KD			  0.15		 // zuo电机速度导数系数
#define R_SPEED_KP            1.5f       // 右电机速度比例系数
#define R_SPEED_KI            0.4f       // 右电机速度积分系数
#define R_SPEED_KD			  0.15		 // 右电机速度导数系数
//右电机速度导数系数
...
//目标速度限幅
#define TARGET_SPPED_MIN 20 //拐弯时最终目标速度=target速度×拐弯系数-差速，不应小于20
#define TARGET_SPEED_MAX 500 //拐弯时最终目标速度=target速度×拐弯系数-差速，不应大于500
// PWM前馈值
#define PWM_QianKui 0

// 过弯减速系数
#define PWM_GuoWanEnble		1			//是否开启过弯减速，为0关闭，为1开启	
#define PWM_GuoWanXiShu		0.5			//过弯减速系数
#define PWM_GuoWanYuZhi		5			//光电管过弯判断阈值

//PID模式选择
#define PID_Mode 1 //PID_Mode == 0 使用三PID串联； PID_Mode == 1 使用二PID * 2并联

//运行时间
#define TIMES_MAX 10000 //运行控制函数这个数后将目标速度归零
...
int about_times = 0;
int tune_flag = 0;	//编码器模式flag，0不输出，1左，2右
...
{
	if((about_times<510)&&((left_encoder!=0)||(right_encoder!=0))){										 
		about_times++;
	}
	...
	switch(tune_flag)
	{
		...
		case 1:
			if(about_times>500 && (left_encoder>20||left_encoder<-20)){
				//更新左电机速度平均值
...
//
		if (motor_left.pwm_output >= 0) {
				TIM1->CCR1 = motor_left.pwm_output + PWM_QianKui, HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, GPIO_PIN_RESET);
		} 
		else {
				TIM1->CCR1 = -motor_left.pwm_output + PWM_QianKui, HAL_GPIO_WritePin(GPIOA, GPIO_PIN_10, GPIO_PIN_SET);
		} 
...
void parallel_pid_control(uint16_t mux_value, float gyro_z, float left_encoder, float right_encoder) {
	...
	//判断是否转弯和差速限幅
if(my_abs(differential_speed) > (TARGET_SPEED / PWM_GuoWanYuZhi)){
    condition_flag = 1;
}
	// 根据位置偏差计算左右电机的目标速度
   if(condition_flag == 0){
...
motor_left.target_speed = constrain(
    TARGET_SPEED * PWM_GuoWanXiShu - differential_speed,
    TARGET_SPPED_MIN, TARGET_SPEED_MAX
);
		//motor_left.target_speed = TARGET_SPEED * PWM_GuoWanXiShu - differential_speed;
motor_right.target_speed = constrain(
    TARGET_SPEED * PWM_GuoWanXiShu + differential_speed,
    TARGET_SPPED_MIN, TARGET_SPEED_MAX
);
		//motor_right.target_speed = TARGET_SPEED * PWM_GuoWanXiShu + differential_speed;
