...
uint8_t WanDaoCheck(uint16_t mux_value,uint8_t active){
	uint8_t result = 0;
	if(my_abs(weighted_sum)>9.0){
		result = 2;
	}
	return result;
}
...
void Update_Condition(uint16_t mux_value, uint8_t active_count){
	switch(condition_now){
		case ZhiDao:{
			...
			else if(WanDaoCheck(mux_value,active_count)){
				condition_now = During_WanDao;
			}
		}
		case During_ZhiJiao:{
		...
		case During_WanDao:{
			if(WanDaoCheck(mux_value ,active_count) == 0){
				condition_now = ZhiDao;
			}	
		}
	}
...
float calculate_line_position(uint16_t mux_value) {
   weighted_sum = 0.0f;
    ...
    if (active_count == 0) {
		if(last_position_error == 0.0) return 0;
...
void parallel_pid_control(uint16_t mux_value, float gyro_z, float left_encoder, float right_encoder) {
	...
	switch(condition_now){
		...
		case During_WanDao: break; //differential_speed *=1.5;
